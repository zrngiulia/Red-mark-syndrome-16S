...
# Load the counts
ta <- read.table("./gemelli_asv_OUTLIERS_rarefied.tsv", header=TRUE, sep="\t", check.names=FALSE, row.names = 1)

# Export the table
exportTable(ta, 'gemelli_asv_OUTLIERS_rarefied.tsv')
...
data = biom$load_table('gemelli_asv_OUTLIERS_rarefied.tsv')
...
metadata <- read.table("./all_metadata_filt_OUTLIERS_rarefied.csv", header=TRUE, sep=",", check.names=FALSE, row.names = 1) #Load metadata
attr(ord.gemelli, "metadata") <- metadata  # Add metadata to the RDA object

# Save RDS to load in the plotting script
saveRDS(ord.gemelli, file = "ord-gemelli-asv-rare-OUTLIERS-rda.rds")
saveRDS(distmatrix.gemelli, file = "distmatrix-gemelli-asv-OUTLIERS-rare.rds")
