...
# Load the counts
ta <- read.table("./gemelli_asv_no_outliers_rarefied.tsv", header=TRUE, sep="\t", check.names=FALSE, row.names = 1)

# Export the table
exportTable(ta, 'gemelli_asv_rare.tsv')
...
data = biom$load_table('gemelli_asv_rare.tsv')
...
metadata <- read.table("./all_metadata_filt_no_outl_rarefied.csv", header=TRUE, sep=",", check.names=FALSE, row.names = 1) #Load metadata
attr(ord.gemelli, "metadata") <- metadata  # Add metadata to the RDA object

# Save RDS to load in the plotting script
saveRDS(ord.gemelli, file = "ord-gemelli-asv-rare-rda.rds")
saveRDS(distmatrix.gemelli, file = "distmatrix-gemelli-asv-rare.rds")
